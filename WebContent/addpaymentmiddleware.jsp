...
String dbpsw = "";
String sql="update loans set Amt_paid= Amt_paid + Monthly_inst  where LoaneeID=?";
...
if( i!=0)
{
		...
		sql="update duepayments set IsPaid= 1 where PaymentID=?";
		Class.forName(driverName);
		con = DriverManager.getConnection(url, user, dbpsw);
		ps = con.prepareStatement(sql);
		...
		if( i!=0)
		{
			response.sendRedirect("allpending.jsp?Id="+LoaneeID	);
		}
		else
		{
			out.println("err");
